...
export async function updateInstanceRole(endpoint: OpsEndpoint, id: string, role: string): Promise<void> {
  ...
  const { changed, skipped } = readWriteResult(parsed, "update_hashes");
  if (changed === null || !changed.includes(id)) {
    throw new Error(
...
export async function deleteInstanceRow(endpoint: OpsEndpoint, id: string): Promise<void> {
  ...
  const { changed, skipped } = readWriteResult(parsed, "deleted_hashes");
  if (changed === null || !changed.includes(id)) {
    throw new Error(
