...
- **Local client: the `npx` stdio adapter.** `@tpsdev-ai/flair-mcp` is a separate package and is not installed globally. `flair init` writes `npx -y @tpsdev-ai/flair-mcp@<version>` into each client's config, pinned to the CLI's own version. The adapter talks to the instance over HTTP with the agent's Ed25519 key. This is the path for Claude Code, Cursor, Codex, Gemini, and every other local MCP client.
- **Remote client: native `/mcp`.** A JSON-RPC endpoint inside the server, OAuth bearer, exposing 12 curated tools. It is **off by default**: until `FLAIR_MCP_OAUTH` is set and the issuer is a public HTTPS origin (`FLAIR_MCP_ISSUER`, else `FLAIR_PUBLIC_URL`), no `/mcp` route is registered and the path returns 404. OAuth client-metadata is fetched over HTTPS and refuses a private, loopback, or link-local host, and `flair mcp enable` refuses a local origin (localhost, loopback, RFC1918, link-local, `.local`). A loopback install cannot be that origin, so native `/mcp` is remote-only by design.
