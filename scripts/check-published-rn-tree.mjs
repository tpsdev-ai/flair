 ...
 *
 * This gate installs a packed tarball as a *dependency* in a throwaway
 * project via npm 12, then asserts:
 ...
 *   2 — DID NOT RUN (no tarball, npm 12 unavailable, install failed)
 *
 * Usage:
...
export function parseArgs(argv) {
  const out = { tree: null, tarball: null, npm: null };
...
export function resolveNpm12(explicit, spawn = spawnSync) {
  if (explicit) {
    const v = spawn(explicit, ["--version"], { encoding: "utf8" });
  ...
  const npx = spawn("npx", ["--yes", NPM12_SPEC, "--version"], { encoding: "utf8" });
  if (npx.status !== 0) {
    return {
      ok: false,
...
export function run(argv = process.argv.slice(2), io = { log: console.log, err: console.error }, spawn = spawnSync) {
  const args = parseArgs(argv);
  if (args.help || (!args.tree && !args.tarball)) {
    io.err(
      "Usage: node scripts/check-published-rn-tree.mjs --tarball <packed.tgz> | --tree <node_modules> [--npm <npm12>]",
    );
  ...
  let measured;
  if (args.tarball) {
    const npmSpec = resolveNpm12(args.npm, spawn);
    if (!npmSpec.ok) {
