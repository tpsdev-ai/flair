- **Unattended ADK auto-promote enforces four authz invariants server-side.**
  ...
  which the promoted memory then carries, so a claim is retrievable by its own
  user's tag filter and invisible to another user's (a tagless promotion into
  the shared agentId namespace is refused, not written); (3) the claim is
