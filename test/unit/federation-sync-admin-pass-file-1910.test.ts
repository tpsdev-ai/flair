/**
 * `flair federation sync` resolves the admin password through the ONE resolver
 * (`resolveAdminPassFromSources`), the same one `federation token`/`pair` use
 * (flair#1909/#1910). Before this it used the older `applyAdminPassFile`, which
 * let `--admin-pass` silently win over `--admin-pass-file` and then fell back
 * to `opts.adminPass ?? env` at the call sites.
 *
 ...
 * is now a usage error. Each test drives the real CLI against a stand-in
 * server that records the Basic credential it was sent, so "authenticates"
 * means the credential actually left the process.
 */
...
const FILE_PASS = "file-secret-federation-1910-sync";
const INLINE_PASS = "inline-secret-federation-1910-sync";
const ENV_PASS = "env-secret-federation-1910-sync";
...
beforeEach(() => {
  dir = join(tmpdir(), `flair-fed-sync-1910-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    async fetch(req) {
      ...
      if (url.pathname === "/FederationPeers") {
        return Response.json({
          peers: [{ id: "hub_1910", role: "hub", status: "active", endpoint: `http://127.0.0.1:${server.port}` }],
        });
      }
      if (url.pathname === "/FederationInstance") {
        return Response.json({ id: "spoke_1910", role: "spoke", publicKey: "spoke-pubkey-1910" });
      }
      switch (body?.operation) {
        case "search_by_conditions":
          // No federable rows → sync completes with nothing to push.
          return Response.json([]);
        case "search_by_value":
          // No legacy key seed on the stand-in.
          return Response.json([]);
        default:
...
describe("federation sync — --admin-pass-file (flair#1910)", () => {
  const syncArgs = (extra: string[]) => [
    "federation",
    "sync",
    "--target",
  ...
  test("reads the password from an owner-only --admin-pass-file", async () => {
    const { stderr, exitCode } = await runCli(syncArgs(["--admin-pass-file", passFile(0o600)]));
  ...
  test("a 0644 file is refused naming the mode, and nothing is sent", async () => {
    const { stdout, stderr, exitCode } = await runCli(syncArgs(["--admin-pass-file", passFile(0o644)]));
  ...
  test("file + --admin-pass together is a usage error, and nothing is sent", async () => {
    const { stdout, stderr, exitCode } = await runCli(
      syncArgs(["--admin-pass-file", passFile(0o600), "--admin-pass", INLINE_PASS]),
    );
    expect(exitCode).not.toBe(0);
  ...
  test("--admin-pass still authenticates", async () => {
    const { stderr, exitCode } = await runCli(syncArgs(["--admin-pass", INLINE_PASS]));
    expect(stderr).not.toContain(INLINE_PASS);
    expect(exitCode).toBe(0);
    const auths = opsAuths();
    expect(auths.length).toBeGreaterThan(0);
  ...
  test("FLAIR_ADMIN_PASS alone still authenticates (the CI form)", async () => {
    const { stderr, exitCode } = await runCli(syncArgs([]), { FLAIR_ADMIN_PASS: ENV_PASS });
  ...
  test("--help prefers the file form and warns that --admin-pass leaks", async () => {
    const { stdout, exitCode } = await runCli(["federation", "sync", "--help"]);
    expect(exitCode).toBe(0);
