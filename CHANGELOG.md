...
- **Default-OFF feature flag (`FLAIR_MCP_ENABLED`).** Off by default; when off the emitted root config is **byte-identical** to today (no `mcp` key → `/mcp` 404s) and FlairMcp's tool-registration nudge is a zero-cost no-op. `resources/mcp-curation.ts:mcpEnabled()`.
- **Config placement is ROOT, not component (the key finding).** Harper's `handleApplication` reads the `mcp:` block from the ROOT config (`getConfigObj()` → `harper-config.yaml` + `HARPER_SET_CONFIG`), NOT flair's component `config.yaml`. So the block is injected at BOTH root-config write points in `src/cli.ts` — the init/start `HARPER_SET_CONFIG` env and the launchd plist `setConfig` — via `mcpConfigBlock()`, spread in only when the flag is on.
- **CRITICAL security — curation is RESOURCE-level, not config-level.** Naively enabling `/mcp` auto-exposes EVERY exported Harper Resource as `get_*`/`search_*`/`create_*`/`update_*`/`delete_*` tools — **147 in the boot, including raw `create_`/`update_`/`delete_` mutators on Agent, Credential, OAuth\*, Federation\*, etc.** The `mcp.application.allow`/`deny` config is **NOT honored for the application profile in Harper 5.1.14** (only the *operations* profile reads it; the application allow/deny params exist in `hdbTerms` but are never read — verified by grep). So the enforced boundary is at the resource level: every flair Resource is `static hidden = MCP_HIDDEN` (override-proof; schema `@export(mcp:false)` is added as defense-in-depth on the auto-generated tables) EXCEPT the single curated `FlairMcp`. See `resources/mcp-curation.ts`.
...
- **Tests.** New `test/integration/mcp-surface.test.ts` boots a real Harper and proves: flag ON → `tools/list` is EXACTLY the 9 (NOT the 147, ZERO mutators), unauthed `tools/call` → "authentication required"; flag OFF → POST `/mcp` 404s. New `test/unit/mcp-curation.test.ts` covers the flag parsing + suppression marker. Full unit suite (1174) + REST/auth/scoping/journey/smoke integration tests green — the schema `@export(mcp:false)` change does not affect REST export.
