...
- **The impl-term-leak gate no longer treats English `ops-*` compounds as bead IDs**
  ...
  exact-literal allowlist — not a heuristic. A real bead ID still
  fails. Each finding now names the matched token and the rule that tripped,
  e.g. `docs/integrations.md:41: matched bead-ID pattern on token "<id>"`.
...
- **Hybrid search no longer reports the RRF rank-normalized value as `_score`
  ...
  similarity failed open at maximal confidence: the flair-client from before 0.18
...
- New unit test for ops socket posture: the posture helper in both postures, lockstep both directions, group-name validation (valid/invalid/missing + not-a-member + broad-group), and all six doctor matrix rows — in-memory fs and mocked group resolution, no real socket or `~/.flair` touched.
...
Per the versioning policy before 1.0, this minor bump is breaking on purpose.
...
### 🧹 Cleanup
- **Removed `flair migrate-keys`:** the `~/.tps/secrets/flair/` layout only existed while Flair lived in the TPS monorepo before 0.1. No published user ever had that path, so the CLI command was dead code from an external perspective. Anyone still sitting on the old layout can migrate manually: `mv ~/.tps/secrets/flair/<agent>-priv.key ~/.flair/keys/<agent>.key` (strip the `-priv` suffix) and run `flair doctor` to confirm.
