...
### 🔌 Native MCP surface — flair speaks MCP at `/mcp`, curated to 9 tools, default-OFF (ops-b6uk, slice 1 of native-MCP+OAuth)

Flair can now expose Harper's native MCP "application" profile at `/mcp` so MCP clients connect WITHOUT the local `@tpsdev-ai/flair-mcp` stdio proxy. This slice lands the reviewable, curated **surface** only — **feature-flagged default-OFF**; it does NOT turn `/mcp` on in prod and does NOT build the Bearer/OAuth verifier (slice 2/3, gated on HarperFast/oauth#86). Until the verifier lands and Sherlock signs off on live enablement, the surface mounts, lists the 9 curated tools, and **rejects every `tools/call` as unauthed**.

- **Default-OFF feature flag (`FLAIR_MCP_ENABLED`).** Off by default; when off **no `HARPER_CONFIG` is emitted** — the spawn env, launchd plist, and deploy `.env` are **byte-identical** to today (no `mcp` key → `/mcp` 404s) and FlairMcp's tool-registration nudge is a zero-cost no-op. `resources/mcp-curation.ts:mcpEnabled()`.
- **Config mechanism — `HARPER_CONFIG` env var, NOT `HARPER_SET_CONFIG` / `harper-config.yaml` (Fabric-compatible).** Harper's MCP `application` profile mounts only when the **resolved** config carries `mcp.application` with ≥1 real key (Harper strips an empty `mcp.application:{}`, so we emit `mountPath`); flair's component `config.yaml` is NOT read for this. The block is delivered via **`HARPER_CONFIG`** — Harper's recommended merge-on-top config var (precedence `HARPER_DEFAULT_CONFIG < config file < HARPER_CONFIG < HARPER_SET_CONFIG`; reasserts its named keys each boot) — set as a **real process env var before boot**, and we **never write to `harper-config.yaml`** (which must not be touched in Fabric). Two delivery paths, both gated on the flag via `src/cli.ts:mcpConfigJson()`: the local/launchd path sets `HARPER_CONFIG` directly in the spawn env / plist `EnvironmentVariables` (`withMcpHarperConfig()`); the Fabric path writes `HARPER_CONFIG=…` into the deploy `.env` (`buildDeployTarball`), which Fabric sources into the process environment before launching Harper — the same `.env` contract `HDB_ADMIN_PASSWORD` has used in prod since #302. `config.yaml` now declares `loadEnv` **first** (Harper requirement) for the app-root `.env`. NOTE the `mcp` block does **not** ride loadEnv: loadEnv is a component plugin that runs *after* Harper resolves config, so a `HARPER_CONFIG` it sets lands too late to mount `/mcp` — verified empirically on pinned Harper 5.1.14 (`HARPER_CONFIG` as a real env var → `/mcp` mounts (200); delivered only via the loadEnv component → 404).
- **CRITICAL security — curation is RESOURCE-level, not config-level.** Naively enabling `/mcp` auto-exposes EVERY exported Harper Resource as `get_*`/`search_*`/`create_*`/`update_*`/`delete_*` tools — **147 in the boot, including raw `create_`/`update_`/`delete_` mutators on Agent, Credential, OAuth\*, Federation\*, etc.** The `mcp.application.allow`/`deny` config is **NOT honored for the application profile in Harper 5.1.14** — re-verified: `buildApplicationTools` filters only by `exportTypes`(`mcp`≠`false`) + `ResourceClass.hidden===true` + verb/`mcpTools` presence; `mcp_application_allow`/`deny` exist only as `hdbTerms` definitions and are read nowhere (only the *operations* profile honors allow/deny). So we deliberately do **not** emit an `mcp.application.allow` list — it would be dead config. The enforced boundary is at the resource level: every flair Resource is `static hidden = MCP_HIDDEN` (override-proof; schema `@export(mcp:false)` is added as defense-in-depth on the auto-generated tables) EXCEPT the single curated `FlairMcp`. See `resources/mcp-curation.ts`.
- **`FlairMcp` — the curated 9-tool surface (`resources/FlairMcp.ts`).** Exposes EXACTLY the 9 tools the `@tpsdev-ai/flair-mcp` proxy ships — `memory_search · memory_store · memory_get · memory_delete · bootstrap · soul_set · soul_get · flair_workspace_set · flair_orgevent` — via `static mcpTools`, each WRAPPING the existing handler (`SemanticSearch`/`Memory`/`MemoryBootstrap`/`Soul`/`WorkspaceState`/`OrgEvent`) — no business logic re-implemented. Per-agent scoping/ownership is inherited from the handlers' `resolveAgentAuth`. Inherited REST verb methods are stripped from FlairMcp's prototype so no `create_FlairMcp` mutator is auto-generated. Each tool requires a verified flair agent on the request context (set by the future verifier) and rejects unauthed in this slice.
- **Workaround for a Harper 5.1.14 timing gap (filing upstream, repro-first).** The native application-tool registry is built when the `mcp` component loads and re-runs ONLY on a schema-graph change — a jsResource-authored `static mcpTools` registers afterward and never gets enumerated (`tools/list` would return `[]`). FlairMcp nudges a re-registration after its module loads (gated on the flag, fully guarded, idempotent). Named as a workaround; to be filed against Harper with the repro.
- **Tests.** `test/integration/mcp-surface.test.ts` boots a real Harper and delivers the `mcp` block via **`HARPER_CONFIG` set as a real env var** (mirroring the production local/Fabric delivery), proving: flag ON → `tools/list` is EXACTLY the 9 (NOT the 147, ZERO mutators), unauthed `tools/call` → "authentication required"; flag OFF (no `HARPER_CONFIG`) → POST `/mcp` 404s. `test/unit/mcp-curation.test.ts` covers the flag parsing + suppression marker. Full unit suite (1174) + full integration suite (132, incl. REST/auth/scoping/journey/HNSW) green — the schema `@export(mcp:false)` change does not affect REST export.
