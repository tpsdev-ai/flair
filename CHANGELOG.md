...
- **A stock `npm install -g @tpsdev-ai/flair` printed a 4-warning deprecation wall and pulled the entire React Native toolchain into every server install.** Root cause: `@harperfast/harper`'s SQL engine dependency `alasql` ships `react-native-fs` as an `optionalDependency` (for its browser/React-Native compatibility mode), and npm installs optionals by default — so every install dragged in `react-native@0.84.1` plus `@react-native/community-cli-plugin`, `dev-middleware`, `chromium-edge-launcher`, `babel-jest`, etc. (255 extra packages, 161MB), and every one of the 4 deprecation warnings (`inflight@1.0.6`, `rimraf@3.0.2`, `glob@7.2.3` ×2) lived in that subtree. `alasql`'s React Native branch is dead code on a server (guarded by a `navigator.product === 'ReactNative'` check that's never true under Node) — verified in a clean tarball-install repro (851→596 packages, 605M→444M, 4→0 deprecation warnings, `npm ls react-native` empty). Fixed with a root `overrides` entry aliasing `react-native-fs` to the tiny inert `empty-npm-package` (no scripts, 362 bytes) — the same lever the issue's original repro tried and found "didn't take effect"; re-tested here against current npm (12.0.1) and it works cleanly.
- **Every flair install also carried a second, differently-versioned copy of Harper.** `@harperfast/oauth`'s `peerDependencies` declares bare `harper` (a distinct, identically-sourced npm publish of the same Harper codebase under an unscoped name) rather than the scoped `@harperfast/harper` flair already bundles — npm's peer auto-install then materialized a full second `node_modules/harper` at a different, uncoordinated version (`5.1.23` vs our pinned `5.1.22`). Added an `overrides` entry pinning `harper` to `npm:@harperfast/harper@5.1.22` — the exact version flair already ships and tests against, eliminating version drift between the two copies (confirmed via `import()` smoke test and the full `@harperfast/oauth` unit + e2e suite, including a real Harper-server boot, all passing unchanged). Note: under npm this still occupies a second on-disk directory (name-based module resolution requires a `harper`-named path to exist), so it doesn't reduce install size on its own — but under this repo's own package manager (bun) the aliased entry fully dedupes to zero extra footprint. Full elimination under npm needs an upstream fix (`@harperfast/oauth` peering on `@harperfast/harper` instead of bare `harper`) — filed as a follow-up rather than forced here.
