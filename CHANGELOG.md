...
Deleted the March-2026 prototype observatory surface (`resources/IngestEvents.ts`, `ObsOffice.ts`, `ObsAgentSnapshot.ts`, `ObsEventFeed.ts`, `ObservationCenter.ts`, `src/observatory-sync.ts`, `ui/observation-center.html`, the three `Obs*` tables in `schemas/schema.graphql`, and their dedicated tests) alongside its allow-list/role wiring (`auth-middleware.ts`'s `/ObservationCenter` public early-return, `cli.ts`'s `ObsOffice`/`ObsAgentSnapshot`/`ObsEventFeed` role grants). The surface was unused in this repo — no code path produces a request to Flair's own `/IngestEvents`, and production observability now runs on the standalone `tpsdev-ai/observatory` app — so this removes dead attack surface (`IngestEvents`' signature check only covered replay via a timestamp window, no nonce store) rather than changing any live behavior.
