...
4. The spoke writes a `Peer` record pointing to the hub so sync knows where to push. A missing hub `publicKey` is an error (or a `GET /FederationInstance` of an existing identity), never an empty string. That does not create the hub's Instance row.
