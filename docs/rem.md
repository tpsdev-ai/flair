...
- **Fail-closed tag lineage.** A candidate is promoted only if it carries an authoritative `adk:<app>:<user>` scope tag, which the promoted memory then carries — retrievable by that user's tag filter, invisible to another user's. A candidate whose scope tag is absent or blank is left pending, never promoted tagless into the shared agentId namespace.
