...
| POST | `/FederationPair` | Pairing token + body-sig | Public at the Harper role gate. Handler validates token, signature, anti-replay. Response includes `instance {id, publicKey}` (503 `hub_instance_identity_incomplete` if the hub has no Instance identity). Fabric uses the bootstrap-user triple from `flair federation token`. |
