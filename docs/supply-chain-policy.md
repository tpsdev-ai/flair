...
- **`node-llama-cpp` is our one optional peer** (`peerDependenciesMeta.optional`), pinned exact rather than ranged because the rerank path depends on version-specific context-size semantics. It is the engine behind the default-OFF rerank stage (`FLAIR_RERANK_ENABLED`), lazily imported at one call site, so a default install never downloads it — which also keeps its CUDA and Vulkan prebuilds off hosts that cannot use them (flair#887). Bake-time exposure is zero until an operator installs it deliberately, the same reasoning as `@harperfast/oauth` above. Note this is specifically an **optional** peer: a plain `optionalDependencies` entry would still be installed by default, since "optional" there means "a failed install is non-fatal", not "skipped".
